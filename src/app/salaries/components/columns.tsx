...
export function getColumns(onEdit: (product: ISalaryLog) => void, remove: (index: number) => Promise<boolean>): ColumnDef<ISalaryLog>[] {
  return [
    {
      id: "select",
...

    {
      ...
      header: "Статус",
      cell: ({ row }) => {
        const status = SalaryLogValues[row.getValue<number>("salary_log_status") as SalaryLogStatus];
