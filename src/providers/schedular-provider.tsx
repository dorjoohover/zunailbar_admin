...
import ModalProvider from "./modal-context"; // Define event and state types
...
const schedulerReducer = (
  ...
  switch (action.type) {
    case "ADD_EVENT":
      return { ...state, events: [...state.events, action.payload] as any };
    ...
    case "SET_EVENTS":
      return { ...state, events: action.payload as any };
...
export const SchedulerProvider = ({
  ...
  onAddEvent?: (event: IOrder) => void;
  onUpdateEvent?: (event: IOrder) => void;
  ...
  useEffect(() => {
    if (initialState) {
      dispatch({ type: "SET_EVENTS", payload: initialState as any });
    }
  }, [initialState]);
  ...
  const getEventsForDay = (day: number, currentDate: Date) => {
    return state?.events.filter((event) => {
      const eventStart = new Date(event.order_date ?? new Date());
      const eventEnd = new Date(event.order_date ?? new Date());
  ...
  function handleEventStyling(
    ...
    allEventsInRange.sort((a, b) => {
      const aStart = +a.start_time.slice(0, 2);
      const bStart = +b.start_time.slice(0, 2);
    ...
    if (event.start_time && event.end_time) {
      ...
      const eventStartHour = Math.abs(+event.start_time.slice(0, 2) - 5);
  ...
  function handleUpdateEvent(event: IOrder, id: string) {
